...
% Se usan en requerimientos.tex; van acÃ¡ para que la numeraciÃ³n sea Ãºnica.
\newcounter{rfcounter}
...
esa definici\'on y no la repite.

Esta es la versi\'on vigente del an\'alisis comercial. Se actualiza a medida que
avanza la validaci\'on y que se toman decisiones sobre precios, planes y
segmentos.

\subsection{Estado de esta versi\'on}

El contenido de este documento proviene de la reorganizaci\'on del Documento de
Visi\'on. Se recoge aqu\'i tal como estaba planteado, para servir de punto de
partida al trabajo posterior. Las cifras de precios y costos son preliminares y
deben revisarse antes de darlas por definitivas.
...
\emph{mal resuelto} no pueden considerarse validadas con la muestra actual. La
evaluaci\'on queda en pausa hasta reunir suficientes encuestas y entrevistas,
...
\emph{briefings}, las exportaciones y el acceso program\'atico seg\'un
corresponda.

\subsection{Planes}

La tabla \ref{tab:planes} presenta los planes preliminares, su precio de
referencia y el valor que incluye cada uno.
...
La estimaci\'on corresponde a un escenario de primera etapa con hasta 500
usuarios, ocho actualizaciones mensuales, 50~GB de almacenamiento, 12 horas
